...
int do_bootm (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	struct lmb lmb;
        ...
        kernel_addr = virt_to_phys(CONFIG_SECURE_KERNEL_BASE);
#else
        kernel_addr = CONFIG_SECURE_KERNEL_BASE;
#endif
        rv = Check_Signature( (SB20_CONTEXT *)SECURE_BOOT_CONTEXT_ADDR,
                              (unsigned char*)kernel_addr, CONFIG_SECURE_KERNEL_SIZE - 256,
                              (unsigned char*)(kernel_addr + CONFIG_SECURE_KERNEL_SIZE - 256), 256 );
        if(rv != SB_OK) {
...
#if defined(CONFIG_ENABLE_MMU)
        ramdisk_addr = virt_to_phys(CONFIG_SECURE_ROOTFS_BASE);
#else
        ramdisk_addr = CONFIG_SECURE_ROOTFS_BASE;
#endif
        rv = Check_Signature( (SB20_CONTEXT *)SECURE_BOOT_CONTEXT_ADDR,
                              (unsigned char*)ramdisk_addr, CONFIG_SECURE_ROOTFS_SIZE - 256,
                              (unsigned char*)(ramdisk_addr + CONFIG_SECURE_ROOTFS_SIZE - 256), 256 );
        if(rv != SB_OK) {
