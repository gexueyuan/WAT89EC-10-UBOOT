...
raw_area_t raw_area_control_emmc;

int init_raw_area_table (block_dev_desc_t * dev_desc)
{
	struct mmc *host;
	int boot_device, device = dev_desc->dev;	
 	host = find_mmc_device(device);
	
	boot_device = get_active_device();
	if(boot_device < 0){
	...
	if (((raw_area_control.magic_number != MAGIC_NUMBER_MOVI)||(raw_area_control_emmc.magic_number != MAGIC_NUMBER_MOVI))) {
		int i = 0;
		member_t *image,temp_image;
		...
		case 2:
			...
			dev_desc->block_read(dev_desc->dev,
			capacity - (eFUSE_SIZE/MOVI_BLKSIZE) - 1,
			1, &raw_area_control_emmc);
			if (raw_area_control_emmc.magic_number == MAGIC_NUMBER_MOVI) {
			return 0;
			}
		
			dbg("Warning: cannot find the raw area table(%p) %08x\n",
				&raw_area_control_emmc, raw_area_control_emmc.magic_number);
			/* add magic number */
			raw_area_control_emmc.magic_number = MAGIC_NUMBER_MOVI;

			/* init raw_area will be 16MB */
			raw_area_control_emmc.start_blk = 16*1024*1024/MOVI_BLKSIZE;
			raw_area_control_emmc.total_blk = capacity;
			raw_area_control_emmc.next_raw_area = 0;
			strcpy(raw_area_control_emmc.description, "initial raw table");

			image = raw_area_control_emmc.image;
			break;	
	
		}
	switch(device){
		case 0: //now 0 and 1 are SD/MMC
		case 1:
		...
		/* image 3 should be u-boot */
		//environment goes to user partition in case of EMMC 4.4
			image[3].start_blk = image[2].start_blk; //write u-boot from block 32 in boot partition.
			image[3].used_blk = MOVI_BL2_BLKCNT_EMMC_4_4;
			image[3].size = PART_SIZE_BL_EMMC_4_4;
			image[3].attribute = 0x2;
			strcpy(image[3].description, "u-boot");
			dbg("bl2: %d\n", image[3].start_blk);
...
#if defined(CONFIG_EMMC_4_4) //bl1 image starts at block 0 of boot partition in emmc 4.4
	#if defined(CONFIG_SECURE_BOOT)
                image[1].start_blk = image[0].start_blk + MOVI_FWBL1_BLKCNT;
        #else
		image[1].start_blk = 0;	
...
int do_movi(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	int mmc_device = get_active_device(); 
	switch(mmc_device){
		case 0:
		case 1: //pls change this hardcoding :)
			image = raw_area_control.image;
			break;
		case 2:
			image = raw_area_control_emmc.image;
	...
	case 'i':
		...
		run_command("mmcinfo", mmc_device);
		return 1;
	case 'r':
		rw = 0;	/* read case */
		break;
	...
	if (attribute == 0x0) {
		...
		sprintf(run_cmd,"mmc %s %d 0x%lx 0x%lx 0x%lx",
				rw ? "write":"read",mmc_device,
				addr, start_blk, blkcnt);
                #ifdef CONFIG_EMMC_4_4 //envi ++
		if(mmc_device == EMMC_DEV){
			emmc_init(mmc);
			emmc_boot_open(mmc);
		}
	...
	if (attribute == 0x2) {
...
#else
		if (rw) {
			start_blk = image[1].start_blk;
			blkcnt = image[1].used_blk;
		...
		sprintf(run_cmd,"mmc %s %d 0x%lx 0x%lx 0x%lx",
				rw ? "write":"read",mmc_device,
				addr + 32768, start_blk, blkcnt); //32 k is signed bl2 appended to u-boot
#else	
		sprintf(run_cmd,"mmc %s %d 0x%lx 0x%lx 0x%lx",
				rw ? "write":"read",mmc_device,
				addr, start_blk, blkcnt);
	...
	#else
		sprintf(run_cmd, "mmc %s %d 0x%lx 0x%lx 0x%lx",
			rw ? "write" : "read",mmc_device, addr, start_blk, blkcnt);
	#endif
	        #ifdef CONFIG_EMMC_4_4 //envi ++
		if(mmc_device == EMMC_DEV){
			mmc = find_mmc_device(mmc_device);
